...
import { Search, Plus, Calendar, Inbox, Volume2, X, Settings2 } from 'lucide-react';
import type { Task, ViewFilter, ThemeId } from '../types';
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-24 px-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-150">
      <div className="w-full max-w-xl glass-panel rounded-2xl border border-white/15 shadow-2xl overflow-hidden flex flex-col bg-[#16181e]">
        ...
        <div className="max-h-80 overflow-y-auto p-2 space-y-1 text-xs">
          ...
          {onSelectTheme && (query.toLowerCase().includes('тем') || query.toLowerCase().includes('theme')) && (
            <>
              ...
              {THEME_OPTIONS.map((th) => (
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-1.5 rounded-xl text-slate-200 hover:bg-white/10 hover:text-white transition"
                >
