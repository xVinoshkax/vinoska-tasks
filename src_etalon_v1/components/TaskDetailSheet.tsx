...
import type { Task, Project, CustomPriority, CustomStatus, RecurringType } from '../types';
...
export const TaskDetailSheet: React.FC<TaskDetailSheetProps> = ({
  ...
  React.useEffect(() => {
    if (isPomodoroRunning) {
      ...
      document.title = `[ ${mins}:${secs < 10 ? '0' : ''}${secs} ] Фокус | ${task?.title || 'Linear Lite'}`;
    } else {
      document.title = 'Linear Lite | Трекер задач';
    }
  ...
  const handleStatusChange = (status: string) => {
    ...
    const isDone = Boolean(matchedStatus?.isDone);
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            >
              ...
              />
              <span className="font-medium truncate max-w-[90px]">{currentStatus?.name || 'Статус'}</span>
